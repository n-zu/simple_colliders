class Line {
    ...
    _checkCircleCollision(other) {
        ...
        const y3 = other.p[1];
        const r = other.r;
        const u = ((x3 - x1) * (x2 - x1) + (y3 - y1) * (y2 - y1)) /
            ((x2 - x1) * (x2 - x1) + (y2 - y1) * (y2 - y1));
        const closestPoint = u < 0
            ? [x1, y1]
            : u > 1
                ? [x2, y2]
    ...
    collideWithCircle(circle) {
        ...
        const m1 = (dpNorm1 * (this.mass - circle.mass) + 2 * circle.mass * dpNorm2) /
            (this.mass + circle.mass);
        const m2 = (dpNorm2 * (circle.mass - this.mass) + 2 * this.mass * dpNorm1) /
            (this.mass + circle.mass);
